...
def mock_plant():
    ...
    plant.simulate.return_value = [
        {"components": {"digester1": {"Q_ch4": 1.0, "pH": 7.0, "VFA": 100.0, "TAC": 5.0}}},
        {"components": {"digester1": {"Q_ch4": 1.1, "pH": 7.1, "VFA": 110.0, "TAC": 5.1}}},
    ]
    comp = MagicMock()
...
def sample_measurements():
    df = pd.DataFrame(
        {"Q_ch4": [1.05, 1.08], "pH": [7.05, 7.08], "VFA": [105, 108]}, index=pd.date_range("2024-01-01", periods=2, freq="h")
    )
...
class TestCalibrationValidator:
    ...
    def test_analyze_residuals(self, mock_plant, sample_measurements):
        validator = CalibrationValidator(mock_plant, verbose=False)
        simulated = {"Q_ch4": np.array([1.0, 1.1, 1.2]), "pH": np.array([7.0, 7.1, 7.2])}
        df = pd.DataFrame(
            {"Q_ch4": [1.05, 1.08, 1.15], "pH": [7.05, 7.08, 7.15]}, index=pd.date_range("2024-01-01", periods=3, freq="h")
        )
    ...
    def test_cross_validate(self, mock_plant):
        validator = CalibrationValidator(mock_plant, verbose=False)
        df = pd.DataFrame(
            {"Q_ch4": np.random.rand(10), "pH": np.random.rand(10), "VFA": np.random.rand(10)},
            index=pd.date_range("2024-01-01", periods=10, freq="h"),
        )
        measurements = MeasurementData(df)
        mock_plant.simulate.return_value = [{"components": {"d": {"Q_ch4": 0.5, "pH": 7.0, "VFA": 100}}}] * 5
    ...
    def test_to_dict(self):
        m = ValidationMetrics(
            ...
            mape=5.0,
            me=0.01,
            observations_mean=1.0,
            observations_std=0.1,
            predictions_mean=1.0,
            predictions_std=0.1,
        )
        d = m.to_dict()
